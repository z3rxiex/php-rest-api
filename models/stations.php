<?php
    ...
    class Station {
        ...
        public function data_1hr() {

            //Query
            $query = 'SELECT station.id id, station.StnCode code, station.StnName name, station.MAC mac, ' .
                'station.Latitude latitude, station.Longitude longitude, station.Elevation elevation, ' .
                'station.Dzongkhag location, station.DateInstalled date_installed, ' .
                'CASE WHEN main.HR' . $this->selected_hr . '_id IS NULL THEN 1 ELSE 0 END HR' . $this->selected_hr . '_status, CASE WHEN intdata.UpdateCount IS NULL THEN 0 ELSE intdata.UpdateCount END UpdateCount' . $this->selected_hr . ', ' .
                'CASE WHEN main.LatestLog IS NULL THEN \'\' ELSE main.LatestLog END LatestLog ' . 
                'FROM intstations station ' .
                'LEFT OUTER JOIN intensity_main main on station.MAC = main.MAC and main.LogDate = "' . $this->selected_date . '" ' .
        ...
        public function update() {
            ...
            $this->latitude = htmlspecialchars(strip_tags($this->latitude));
            $this->longitude = htmlspecialchars(strip_tags($this->longitude));
            $this->gewog = htmlspecialchars(strip_tags($this->gewog));
            $this->elevation = htmlspecialchars(strip_tags($this->elevation));
            $this->date_installed = htmlspecialchars(strip_tags($this->date_installed));
            ...
            $result->bindParam(':latitude', $this->latitude);
            $result->bindParam(':longitude', $this->longitude);
            $result->bindParam(':gewog', $this->gewog);
            $result->bindParam(':elevation', $this->elevation);
            $result->bindParam(':date_installed', $this->date_installed);
